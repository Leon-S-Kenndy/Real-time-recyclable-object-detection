package com.hushiyu1995.yolo_v3;
...
public class Yolo extends AppCompatActivity {
    ...
    //创建handler任务，参考：https://www.jianshu.com/p/7e0ca399aea7
    ...
    public Yolo() {
        //默认的source file是dog图片
        srcimgpath = "/sdcard/yolo/data/dog.jpg";
    }

    //  将asserts目录下所有文件(cfg, weights, data)复制到手机上
...
//  @param  newPath  String  复制后路径  如：xx:/bb/cc
    ...
    public void RyoloDetect( ){

        new Thread(new Runnable() {
            public void run() {
                double runtime = Rtestyolo(srcimgpath);
                Log.i(TAG, "yolo run time " + runtime);
                Message msg = new Message();
                msg.what = DETECT_FINISH;
                msg.obj = runtime;
    ...
    public void exactresClick(View v){
        view_status.setText("Extract model, please wait");
        ...
        view_status.setText("Extract model finish");

    }

    public void DanalyseClick(View v){
    ...
    public void RanalyseClick(View v){
    ...
    public native double Rtestyolo(String imgfile);
